...
func TestUpdateTaskUndoneExecute(t *testing.T) {
	...
	t.Run("should be update task to undone", func(t *testing.T) {
		...
		us := UpdateTaskUndoneUseCaseImpl{
		...
		mock.ExpectExec(`UPDATE tb_task SET done = \? WHERE NOT deleted_at AND id = \?`).WithArgs(false, 1).WillReturnResult(sqlmock.NewResult(1, 1))

		err = us.Execute(ctx, UpdateTaskUndoneUseCaseInput{
			UserID: uint32(1),
			UUID:   "uuid",
		})
